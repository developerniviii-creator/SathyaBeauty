...
const Offers = () => {
  ...
  return (
    <div className="container mx-auto px-6 py-12 font-sans relative">
      ...
      <div className="absolute top-20 right-10 w-72 h-72 bg-primary/20 rounded-full blur-3xl -z-10 pointer-events-none"></div>
      <div className="absolute bottom-10 left-10 w-96 h-96 bg-secondary/20 rounded-full blur-3xl -z-10 pointer-events-none"></div>
      ...
      {offers.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 text-center border-2 border-dashed border-primary/30 rounded-3xl bg-amber-50/50 max-w-3xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
          {offers.map((offer) => (
            <div key={offer.id} className="relative bg-white rounded-3xl p-1 shadow-xl shadow-amber-100/50 hover:shadow-2xl hover:shadow-amber-200/60 overflow-hidden group transition-all duration-300 transform hover:-translate-y-1">
              {/* Animated gradient border */}
              <div className="absolute inset-0 bg-gradient-to-r from-primary via-secondary to-primary opacity-20 group-hover:opacity-100 transition-opacity duration-500 -z-10"></div>
              
              <div className="absolute top-0 right-0 bg-gradient-to-r from-primary to-secondary text-white font-bold px-5 py-2 rounded-bl-2xl z-20 shadow-sm">
                Limited Time
              </div>
              ...
              <div className="bg-white rounded-[23px] h-full flex flex-col justify-between relative z-10 overflow-hidden">
                ...
                <div className={`p-8 pt-6 ${offer.image ? '' : 'mt-4'} flex flex-col flex-1`}>
                  {!offer.image && (
                    <h3 className="text-3xl font-extrabold text-gray-800 mb-3 group-hover:text-primary transition-colors">{offer.name}</h3>
                  )}
                  ...
                  {offer.services_included && (
                    <div className="mb-6 flex flex-wrap gap-2">
                      {offer.services_included.split(', ').map((srv, idx) => (
                        <span key={idx} className="bg-amber-50 text-primary text-xs font-bold px-3 py-1.5 rounded-full border border-amber-100">
                          {srv}
                        </span>
                      ))}
                    </div>
                ...
                <div className="p-8 pt-0 flex flex-col sm:flex-row items-center justify-between border-t border-gray-100 mt-2 gap-4">
                  ...
                  <Link 
                    ...
                    className="w-full sm:w-auto text-center bg-gray-900 hover:bg-gradient-to-r hover:from-primary hover:to-secondary text-white font-bold py-3 px-8 rounded-xl transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5 mt-4 sm:mt-0"
                  >
                    Claim Offer
                  </Link>
                </div>
